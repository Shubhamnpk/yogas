...
export function DealerPendingApproval({
  ...
  const pending = dealer.approval_status === "pending";
    ...
    <div className="mx-auto w-full max-w-2xl space-y-6 px-4 py-10">
      <div className="rounded-2xl border border-border bg-card p-8 text-center shadow-soft">
        ...
        <h1 className="mt-4 font-display text-2xl font-bold">
          {pending ? t("common:approvalPendingTitle") : t("common:approvalRejectedTitle")}
        </h1>
        <p className="mx-auto mt-2 max-w-md text-sm text-muted-foreground">
          {pending ? t("common:approvalPendingBody") : t("common:approvalRejectedBody")}
        </p>
        <div className="mt-5 flex flex-col items-center gap-2 text-sm">
          <p className="font-semibold">{dealer.business_name}</p>
          <p className="text-muted-foreground">
            {t("common:appliedOn", { date: formatDateTime(dealer.requested_at) })}
          </p>
          <span className="mt-1 inline-flex items-center gap-1.5 rounded-full bg-secondary px-3 py-1 text-xs font-semibold capitalize text-secondary-foreground">
            ...
            {dealer.is_active ? t("common:active") : t("common:inactive")}
          </span>
        </div>
      ...
      <div className="rounded-2xl border border-border bg-card p-5 shadow-soft">
        <div className="flex items-center gap-2">
          ...
          <h2 className="font-semibold">{t("common:yourActivity")}</h2>
        </div>
        <p className="mt-1 text-sm text-muted-foreground">
          {profile?.full_name ?? t("common:yourAccount")} · {dealer.business_name}
        </p>
