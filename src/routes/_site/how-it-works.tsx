...
export const Route = createFileRoute("/_site/how-it-works")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Learn how YoGas works in three simple steps for consumers and dealers.",
      },
    ],
  }),
...
function HowItWorksPage() {
  ...
  const startSimulation = (e: React.FormEvent) => {
    ...
    setSimMessage(
      lang === "ne"
        ? "लाममा प्रवेश सफल! तपाईँको क्रम #४ हो।"
        : "Queue joined! Your position is #4.",
    );
  };
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      if (nextStep === 2) {
        setSimMessage(
          lang === "ne"
            ? "डिपोमा ग्यास अनलोड भइरहेको छ... क्रम #३।"
            : "Depot unloading truck... position is #3.",
        );
      } else if (nextStep === 3) {
        setSimMessage(
          lang === "ne"
            ? "सिलिन्डर प्रमाणित र तयार गरिँदै... क्रम #२।"
            : "Preparing your cylinder... position is #2.",
        );
      } else if (nextStep === 4) {
        setSimMessage(
          lang === "ne"
            ? "वितरणका लागि तयारी! डिपो काउन्टरमा स्क्यान गर्नुहोस्।"
            : "Ready for collection! Scan this QR at counter.",
        );
      }
    }, 4500);
    ...
    <div className="mx-auto max-w-6xl px-4 py-10 md:py-16 space-y-24">
      ...
      <div className="text-center space-y-4">
        <span className="inline-flex items-center gap-1.5 rounded-full bg-primary/10 px-3 py-1 text-xs font-semibold text-primary">
          ...
          {lang === "ne" ? "डिजिटल वितरण प्रणाली" : "Digital Distribution"}
        </span>
      ...
      <div className="space-y-12">
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          {activeTab === "consumer"
            ? [
                content.consumer.step1,
                content.consumer.step2,
                content.consumer.step3,
                content.consumer.step4,
              ].map((step, i) => (
                <Card
                  key={i}
                  className="relative rounded-2xl border border-border bg-card shadow-soft overflow-hidden hover:shadow-lift transition-all duration-200"
                >
            ...
            : [
                content.dealer.step1,
                content.dealer.step2,
                content.dealer.step3,
                content.dealer.step4,
              ].map((step, i) => (
                <Card
                  key={i}
                  className="relative rounded-2xl border border-border bg-card shadow-soft overflow-hidden hover:shadow-lift transition-all duration-200"
          ...
          <div className="lg:col-span-6 space-y-6">
            ...
            {simStep === 0 ? (
              <form
                ...
                className="space-y-4 bg-card p-6 rounded-2xl border border-border shadow-soft"
              >
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-bold text-foreground">
                      {lang === "ne" ? "काल्पनिक नागरिकता नं" : "Mock Citizenship No."}
                    </label>
                    <Input
              ...
              <div className="bg-card p-6 rounded-2xl border border-border shadow-soft space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span
                    className={`inline-flex items-center gap-1 text-xs font-bold ${simStep === 4 ? "text-success" : "text-primary animate-pulse"}`}
                  >
                    ●{" "}
                    {simStep === 4
                      ? lang === "ne"
                ...
                <p className="text-sm font-semibold text-foreground bg-secondary/50 p-3 rounded-lg border border-border/60">
                  <Megaphone className="size-4 text-primary inline-block mr-1.5 -mt-0.5" />{" "}
                  {simMessage}
                </p>
                <div className="flex justify-between items-center text-xs text-muted-foreground pt-2">
                  <span>
                    {lang === "ne" ? "उपभोक्ता: " : "Consumer: "}
                    <strong>{simName}</strong>
                  </span>
                  <span>
                    {lang === "ne" ? "नागरिकता: " : "Citizenship: "}
                    <strong>{simCitizenship}</strong>
                  </span>
                </div>
            ...
            {simStep === 0 ? (
              <div className="w-72 h-96 border border-dashed border-border rounded-3xl flex flex-col items-center justify-center bg-card/60 p-6 text-center space-y-3">
                ...
                <p className="text-sm font-semibold text-muted-foreground">
                  {lang === "ne"
                    ? "विवरणहरू भरेर सिमुलेशन सुरु गर्नुहोस्"
                    : "Enter details to view mock cylinder pass"}
                </p>
              </div>
            ) : (
                ...
                <div className="p-6 flex-1 flex flex-col items-center justify-center text-center space-y-6">
                  {simStep < 4 ? (
                    <div className="space-y-2">
                      <p className="text-xs text-muted-foreground font-semibold uppercase">
                        {lang === "ne" ? "लामको स्थान" : "WAITLIST POSITION"}
                      </p>
                      <p className="text-6xl font-display font-extrabold text-foreground">
                        #{simQueuePos}
                      </p>
                      <p className="text-xs text-primary font-bold">
                        {lang === "ne" ? "अनुमानित समय: १५-४५ मिनेट" : "Est. Wait: 15-45 mins"}
                      </p>
                    </div>
                  ) : (
                  ...
                  <div className="border-t border-dashed border-border/70 pt-4 w-full text-left space-y-1">
                    ...
                    <p className="text-[10px] text-muted-foreground uppercase font-bold pt-1">
                      {lang === "ne" ? "सिलिन्डर आकार" : "CYLINDER SIZE"}
                    </p>
      ...
      <section className="space-y-12">
        <div className="text-center space-y-3">
          ...
          <p className="text-muted-foreground max-w-xl mx-auto">
            {lang === "ne"
              ? "हामीले ग्यास वितरण प्रक्रियालाई कसरी पारदर्शी र मर्यादित बनायौं"
              : "How we bring transparency and dignity to Nepalese LPG distribution."}
          </p>
        </div>
        ...
        <div className="grid gap-6 md:grid-cols-2 max-w-4xl mx-auto">
          ...
          <div className="border border-border bg-card rounded-2xl p-6 space-y-4">
            <h3 className="text-base font-bold text-destructive flex items-center gap-2">
              ...
              {lang === "ne" ? "परम्परागत सडक लामहरू" : "Traditional Street Queues"}
            </h3>
            <ul className="space-y-3 text-sm text-muted-foreground">
              <li className="flex items-start gap-2">
                <X className="size-4 text-destructive shrink-0 mt-0.5" />
                {lang === "ne"
                  ? "घण्टौंसम्म घाम, पानी वा चिसोमा लाइन बस्नुपर्ने"
                  : "Standing under direct sun or rain for hours"}
              </li>
              <li className="flex items-start gap-2">
                <X className="size-4 text-destructive shrink-0 mt-0.5" />
                {lang === "ne"
                  ? "पक्षपात, भण्डारण र कालोबजारीको उच्च जोखिम"
                  : "High risk of hoarding, back-channel deals, or scalping"}
              </li>
              <li className="flex items-start gap-2">
                <X className="size-4 text-destructive shrink-0 mt-0.5" />
                {lang === "ne"
                  ? "ग्यास स्टक कहिले आइपुग्छ र कति छ भन्ने कुनै जानकारी नहुने"
                  : "Zero visibility into stock delivery timing or exact quantities"}
              </li>
              <li className="flex items-start gap-2">
                <X className="size-4 text-destructive shrink-0 mt-0.5" />
                {lang === "ne"
                  ? "बिचौलियाहरूले कृत्रिम अभाव सिर्जना गर्न सक्ने"
                  : "Middlemen grabbing cards to resell queue spots"}
              </li>
            </ul>
          </div>
          ...
          <div className="border border-primary/30 bg-primary/5 rounded-2xl p-6 space-y-4">
            <h3 className="text-base font-bold text-primary flex items-center gap-2">
              ...
              {lang === "ne" ? "योग्यास पारदर्शी प्रणाली" : "YoGas Virtual Queues"}
            </h3>
            <ul className="space-y-3 text-sm text-foreground/80">
              <li className="flex items-start gap-2">
                <Check className="size-4 text-primary shrink-0 mt-0.5" />
                {lang === "ne"
                  ? "घरमै बसी मोबाइलमा आफ्नो स्थान र पालो प्रत्यक्ष रूपमा हेर्न मिल्ने"
                  : "Track position from home and only go to the depot when ready"}
              </li>
              <li className="flex items-start gap-2">
                <Check className="size-4 text-primary shrink-0 mt-0.5" />
                {lang === "ne"
                  ? "एक नागरिकता नम्बर = एक कोटा लाम, पक्षपात र भण्डारण बन्द"
                  : "One citizenship = one line limit, cutting off hoarding"}
              </li>
              <li className="flex items-start gap-2">
                <Check className="size-4 text-primary shrink-0 mt-0.5" />
                {lang === "ne"
                  ? "डिपोको लाइभ स्टक र दैनिक वितरणको पूर्ण पारदर्शिता"
                  : "Full audit trail of active stock numbers and handovers"}
              </li>
              <li className="flex items-start gap-2">
                <Check className="size-4 text-primary shrink-0 mt-0.5" />
                {lang === "ne"
                  ? "संकलन कोड म्यानुअल वा QR स्क्यान गरी तत्काल प्रमाणित हुने"
                  : "Rapid verification at counter in 3 seconds flat"}
              </li>
            </ul>
          </div>
        </div>
      ...
      <section className="text-center bg-card border border-border rounded-3xl p-8 max-w-xl mx-auto space-y-4">
        ...
        <Button asChild size="lg" className="mt-2">
          <Link to="/auth">
            {lang === "ne" ? "पर्खाइ सूचीमा जोडिनुहोस्" : "Register / Sign In"}
          </Link>
        </Button>
      </section>
    </div>
